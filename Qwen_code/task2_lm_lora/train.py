...
def build_lora_targets(layers: Iterable[int]) -> List[str]:
    modules: List[str] = []
    for idx in layers:
        modules.extend(
            [
    ...
    modules.append("visual.merger.mlp.0")
    modules.append("visual.merger.mlp.2")
...
def build_lora_config(model: Qwen2_5_VLForConditionalGeneration, cfg: TrainConfig) -> LoraConfig:
    ...
    if cfg.lora_target_layers is None:
        target_layers = list(range(len(visual.blocks)))
    else:
        target_layers = [layer for layer in cfg.lora_target_layers if 0 <= layer < len(visual.blocks)]
        if not target_layers:
            raise ValueError("No valid entries provided for lora_target_layers")

    target_modules = build_lora_targets(target_layers)
