...
def run_explanation(cfg: Dict):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Explain", dynamic_ncols=True):
            ...
            for path, label_val, prob_val, pred_val, text_val in zip(paths, label_np, prob_np, pred_np, decoded_texts):
                records.append(
                    {
                        "image_path": str(path),
...
def save_inference(cfg: Dict, records: list[Dict]) -> Path:
    output_path = Path(cfg.get("inference_output", "task5_explanation_results.json"))
...
def main():
    ...
    metrics, records = run_explanation(cfg)
