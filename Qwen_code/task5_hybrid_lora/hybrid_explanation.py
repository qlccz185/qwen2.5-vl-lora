# -*- coding: utf-8 -*-
"""Hybrid Task5 explanation pipeline aligned with the training configuration."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Task5 hybrid explanation")
    default_cfg = Path(__file__).with_name("config_hybrid_explanation.json")
...
def build_dataloader(processor, cfg: Dict) -> DataLoader:
    ...
    prompt_candidates = [
        ...
        for p in cfg.get("prompt_candidates", [])
    ]
    prompt_candidates = [p for p in prompt_candidates if p]
    ...
    def collate_fn(batch):
        if prompt_candidates:
            prompts = [random.choice(prompt_candidates) for _ in batch]
        else:
            prompts = prompt_text
        return collate_joint_test(
            batch,
            processor,
            image_size,
            prompt_text=prompts,
...
def run_explanation(cfg: Dict):
    ...
    output_path = Path(cfg.get("inference_output", "task5_explanation_results.csv"))
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = ["image_path", "image_label", "prompt", "explanation"]
    ...
    with output_path.open("w", encoding="utf-8", newline="") as csvfile:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Explain", dynamic_ncols=True):
                inputs, labels, _, paths, _, prompts = batch
                ...
                for seq in generated:
                    gen_tokens = seq[prompt_len:].detach().cpu().tolist()
                    text = tokenizer.decode(gen_tokens, skip_special_tokens=True).strip()
                    decoded_texts.append(text)

                for path, label_val, prompt_val, text_val in zip(paths, label_np, prompts, decoded_texts):
                    writer.writerow(
                        {
                            "image_path": str(path),
                            "image_label": int(label_val),
                            "prompt": str(prompt_val),
                            "explanation": text_val,
                        }
                    )
                    csvfile.flush()
...
def main():
    ...
    metrics, output_path = run_explanation(cfg)
    if metrics:
