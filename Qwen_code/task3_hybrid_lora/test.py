...
class ForgeryJointValDataset(Dataset):
    """
    支持 trainingset2 和 testset2 两套目录结构的通用 Dataset。
    JSON 中的路径可为:
        - trainingset2/image/xxx.jpg
        - testset2/image/yyy.jpg
    data_root 一律设置为 /root/autodl-tmp/data。
    """
    def __init__(self, ann_path, data_root="/root/autodl-tmp/data"):
        ...
        for rec in items:
            ...
            if y == 1:
                ...
                if mp and isinstance(mp, str) and not ("x" in mp.lower()):
                    mask_p = (self.root / mp).resolve()
                    if not mask_p.exists():
                        print(f"[WARN] mask not found: {mask_p}")
                        mask_p = None
                self.items.append({
                    "path": str(img_p),
                    "label": 1,
        ...
        print(f"[INFO] Loaded {len(self.items)} samples from {ann_path}")
