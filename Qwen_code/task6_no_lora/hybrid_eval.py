...
def parse_args():
    parser = argparse.ArgumentParser(description="Task3 hybrid evaluation without LoRA")
    default_cfg = Path(__file__).with_name("config_no_lora_eval.json")
...
def append_metrics_to_csv(cfg: Dict, metrics: Dict) -> Path:
    csv_path = Path(cfg.get("metrics_csv", "task6_eval_metrics.csv"))
...
def save_inference(cfg: Dict, records: list[Dict]) -> Path:
    output_path = Path(cfg.get("inference_output", "task6_inference_results.json"))
