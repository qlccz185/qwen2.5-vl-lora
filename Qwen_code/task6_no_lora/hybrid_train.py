...
def parse_args():
    parser = argparse.ArgumentParser(description="Task3 hybrid trainer without LoRA")
    default_cfg = Path(__file__).with_name("config_no_lora_train.json")
...
def build_model_and_tools(cfg: Dict):
    ...
    if not frozen_head_path:
        raise ValueError("Configuration must include frozen_head_path for task6 training.")
...
def save_artifacts(cfg: Dict, model, heads, fusion_projector, lm_lora_enabled: bool):
    ...
    if lm_lora_enabled:
        lora_dir = out_dir / cfg.get("lora_checkpoint", "task6_lora")
        ...
        print("[INFO] LM parameters are frozen; no adapter checkpoint will be saved.")
    ...
    fusion_path = out_dir / cfg.get("fusion_checkpoint", "task6_fusion.pt")
